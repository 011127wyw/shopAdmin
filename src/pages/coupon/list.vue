    ...
    <FormDrawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
      ...
      >
        ...
        ><el-form-item label="面值" prop="value">
          <el-input v-model="form.value" style="width: 30%" placeholder="面值">
            <template
              #suffix
              style="font-style: normal; margin-right: 10px; line-height: 30px"
              >{{ form.type == 0 ? "元" : "折" }}</template
            >
          </el-input>
        </el-form-item>
...
const {
  ...
  beforeSubmit: (f) => {
    if (typeof f.start_time != "number") {
      f.start_time = (new Date(f.start_time)).getDate()
    }
    if (typeof f.end_time != "number") {
      f.end_time = (new Date(f.end_time)).getDate()
    }
  }
